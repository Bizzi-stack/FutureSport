...
export function applyMatchContributions(students = [], updatedMatch, options = {}) {
    ...
    const { playerStats } = updatedMatch;
    const matchYear = String(updatedMatch.year || '2026');
    ...
    return students.map(student => {
        let stats = null;
        if (playerStats) {
            const matchedKey = Object.keys(playerStats).find(k => matchesStudentId(k, student));
            if (matchedKey) stats = playerStats[matchedKey];
        }
