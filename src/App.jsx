...
function App() {
  ...
  const [pmcStudents, setPmcStudents] = useState(() => {
    try {
      ['eduvision-pmc-students', 'eduvision-pmc-students-v4', 'eduvision-pmc-students-v5', 'eduvision-pmc-students-v6', 'eduvision-pmc-students-v7', 'eduvision-pmc-students-v8', 'eduvision-pmc-students-v9', 'eduvision-pmc-students-v20', 'eduvision-pmc-students-v20', 'eduvision-pmc-students-v20'].forEach(k => {
        try { localStorage.removeItem(k); } catch {}
      });
      const saved = localStorage.getItem('eduvision-pmc-students-v20');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eduvision-pmc-students-v20', JSON.stringify(pmcStudents));
    } catch {}
  }, [pmcStudents]);
  ...
  const [pmcMatches, setPmcMatches] = useState(() => {
    try {
      ...
      const saved = localStorage.getItem('eduvision-pmc-matches-v20');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          if (didUpdate) {
            localStorage.setItem('eduvision-pmc-matches-v20', JSON.stringify(updatedList));
          }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eduvision-pmc-matches-v20', JSON.stringify(pmcMatches));
    } catch { /* ignored */ }
  }, [pmcMatches]);
  ...
  const handleResetPmcMatches = () => {
    ...
    try {
      localStorage.removeItem('eduvision-pmc-matches-v9');
      localStorage.setItem('eduvision-pmc-matches-v20', JSON.stringify(sanitized));
